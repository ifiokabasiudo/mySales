...
export default function ReconciliationModal({
  ...
  useEffect(() => {
    const batch = items
      .filter((item) => item.item_id == searchId && item.is_active)
      .sort((a, b) => {
  ...
  const handleSubmit = async () => {
    await run(
      async () => {
        ...
        if (tQuantity <= 0) throw new Error("Enter a quantity");
        ...
        if (!inventorySale) {
          throw new Error("Failed to record inventory sale");
        }
      ...
      { success: "Allocation Successful!", loading: "Allocating..." }
    );
  };
        ...
        <div className="relative bg-white w-full max-w-sm rounded-xl p-5 z-10">
          ...
          <div className="flex flex-col gap-2 mt-4">
            <SearchBar
              ...
              onSearch={() =>
                handleSearchAttempt({ name: "Inventory", link: "/inventory" })
              }
            />
            {price > 0 && qty > 0 && (
